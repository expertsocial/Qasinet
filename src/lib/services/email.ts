...
export interface AdminServiceLockAlertParams {
  serviceId: string;
  serviceName: string;
...
/**
 * Sends an alert email to the admin whenever a service status is locked or unlocked.
 */
export async function sendAdminServiceLockAlertEmail(params: AdminServiceLockAlertParams): Promise<SendEmailResult> {
  const adminEmail = process.env.ADMIN_ALERT_EMAIL || 'qasinetltd@gmail.com';
  const baseUrl = process.env.NEXT_PUBLIC_BASE_URL || 'https://qasinet.com';
  const controlUrl = `${baseUrl}/admin/services/control`;
  const formattedDate = params.date
    ? new Date(params.date).toLocaleString('en-KE', { timeZone: 'Africa/Nairobi' })
    : new Date().toLocaleString('en-KE', { timeZone: 'Africa/Nairobi' });

  const isLocked = params.action === 'LOCKED';
  const subject = `${isLocked ? '🔒' : '🔓'} [SERVICE CONTROL] ${params.serviceName} ${params.action} by ${params.adminEmail || 'Admin'}`;
...
<head>
  <meta charset="utf-8">
  <title>Service Control Alert</title>
</head>
<body style="margin: 0; padding: 0; background-color: #0b0f19; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; color: #f8fafc;">
  <div style="max-width: 600px; margin: 30px auto; background-color: #111827; border: 2px solid ${isLocked ? '#f59e0b' : '#10b981'}; border-radius: 16px; overflow: hidden;">
    <div style="background-color: ${isLocked ? '#d97706' : '#059669'}; color: #ffffff; padding: 14px 24px; font-weight: 700; font-size: 14px; text-transform: uppercase; letter-spacing: 1px;">
      ${isLocked ? '🔒 Service Locked by Admin' : '🔓 Service Unlocked / Restored'}
    </div>

    <div style="padding: 24px;">
      <p style="font-size: 15px; color: ${isLocked ? '#fde68a' : '#a7f3d0'}; margin: 0 0 16px; line-height: 1.5;">
        The availability state of <strong>${params.serviceName}</strong> has been updated in live production.
      </p>

      <table style="width: 100%; border-collapse: collapse; background-color: #1e293b; border-radius: 8px; overflow: hidden; font-size: 13px; margin-bottom: 20px;">
        <tr>
          <td style="padding: 10px 14px; color: #94a3b8; border-bottom: 1px solid #334155;">Service:</td>
          <td style="padding: 10px 14px; color: #ffffff; font-weight: 700; border-bottom: 1px solid #334155;">${params.serviceName} (${params.serviceId})</td>
        </tr>
        <tr>
          <td style="padding: 10px 14px; color: #94a3b8; border-bottom: 1px solid #334155;">New Status:</td>
          <td style="padding: 10px 14px; color: ${isLocked ? '#f59e0b' : '#10b981'}; font-weight: 700; text-transform: uppercase; border-bottom: 1px solid #334155;">${params.status}</td>
        </tr>
        <tr>
          <td style="padding: 10px 14px; color: #94a3b8; border-bottom: 1px solid #334155;">Action Taken By:</td>
          <td style="padding: 10px 14px; color: #ffffff; font-family: monospace; border-bottom: 1px solid #334155;">${params.adminEmail || 'Admin'}</td>
        </tr>
        <tr>
          <td style="padding: 10px 14px; color: #94a3b8; border-bottom: 1px solid #334155;">Mandatory Reason:</td>
          <td style="padding: 10px 14px; color: #f1f5f9; border-bottom: 1px solid #334155;">${params.reason}</td>
        </tr>
        ${params.customerFacingMessage ? `
        <tr>
          <td style="padding: 10px 14px; color: #94a3b8; border-bottom: 1px solid #334155;">Customer Notice:</td>
          <td style="padding: 10px 14px; color: #cbd5e1; font-style: italic; border-bottom: 1px solid #334155;">"${params.customerFacingMessage}"</td>
        </tr>
        ` : ''}
        <tr>
      ...
      <div style="text-align: center; margin: 24px 0 12px;">
        <a href="${controlUrl}" target="_blank" style="display: inline-block; background-color: #10b981; color: #022c22; text-decoration: none; font-weight: 700; padding: 12px 24px; border-radius: 8px; font-size: 14px;">
          Manage Services in Admin Panel →
        </a>
      </div>

      <p style="font-size: 11px; color: #64748b; text-align: center; margin-top: 16px;">
        Automatic audit notification generated by QasiNet Service Control.
      </p>
    </div>
  </div>
</body>
